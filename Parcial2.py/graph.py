...
class Graph(List):
    ...
    def insert_edge(self, origin_vertex, destination_vertex, weight):
        origin = self.search(origin_vertex, 'value')
        destination = self.search(destination_vertex, 'value')

        if origin is None or destination is None:
            print("No se puede insertar, falta un vértice")
            return

        # Arista origin → destination
        self[origin].edges.append(Graph.__nodeEdge(destination_vertex, weight))

        # Si el grafo es NO dirigido, agregamos también la inversa
        if not self.is_directed:
            self[destination].edges.append(Graph.__nodeEdge(origin_vertex, weight))
